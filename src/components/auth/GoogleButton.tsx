"use client";

import { useEffect, useRef, useState } from "react";
import {
  GOOGLE_CLIENT_ID,
  GOOGLE_ENABLED,
  getGoogleId,
  GSI_SRC,
  GoogleCredentialResponse,
} from "@/lib/google";

interface GoogleButtonProps {
  onCredential: (idToken: string) => void;
  busy?: boolean;
  text?: string;
}

function GoogleIcon() {
  return (
    <svg className="size-4.5 shrink-0" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
      />
    </svg>
  );
}

export default function GoogleButton({
  onCredential,
  busy = false,
  text = "المتابعة باستخدام Google",
}: GoogleButtonProps) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const [ready, setReady] = useState(false);

  useEffect(() => {
    if (!GOOGLE_ENABLED) return;

    // Load GSI script if not loaded
    if (!document.querySelector(`script[src="${GSI_SRC}"]`)) {
      const script = document.createElement("script");
      script.src = GSI_SRC;
      script.async = true;
      script.defer = true;
      document.body.appendChild(script);
    }

    let cancelled = false;

    const init = () => {
      const gid = getGoogleId();
      const container = containerRef.current;
      if (!gid || !container || cancelled) return false;

      try {
        gid.initialize({
          client_id: GOOGLE_CLIENT_ID,
          callback: (res: GoogleCredentialResponse) => {
            if (res.credential) onCredential(res.credential);
          },
          auto_select: false,
        });

        container.innerHTML = "";
        gid.renderButton(container, {
          theme: "outline",
          size: "large",
          text: "continue_with",
          shape: "pill",
          locale: "ar",
          logo_alignment: "center",
        });

        setReady(true);
        return true;
      } catch {
        return false;
      }
    };

    if (init()) return;

    const poll = setInterval(() => {
      if (init()) clearInterval(poll);
    }, 250);

    const timer = setTimeout(() => clearInterval(poll), 6000);

    return () => {
      cancelled = true;
      clearInterval(poll);
      clearTimeout(timer);
    };
  }, [onCredential]);

  const handleClick = () => {
    if (busy) return;
    const gid = getGoogleId();
    if (gid && GOOGLE_CLIENT_ID && typeof gid.prompt === "function") {
      gid.prompt();
    } else if (!GOOGLE_CLIENT_ID) {
      alert("خدمة تسجيل الدخول عبر Google تتطلب إعداد GOOGLE_CLIENT_ID.");
    }
  };

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <span className="h-px flex-1 bg-[#ede5da]" />
        <span className="text-xs font-bold text-[#80766b]">أو</span>
        <span className="h-px flex-1 bg-[#ede5da]" />
      </div>

      <div className="relative">
        <button
          type="button"
          onClick={handleClick}
          disabled={busy}
          className="flex w-full items-center justify-center gap-2.5 rounded-2xl border border-[#ede5da] bg-[#faf7f2] px-4 py-3 text-xs font-bold text-[#1e1b18] shadow-2xs transition duration-200 hover:border-[#7d1d29]/40 hover:bg-white active:scale-99 disabled:opacity-60"
        >
          <GoogleIcon />
          <span>{text}</span>
        </button>

        {GOOGLE_ENABLED && (
          <div
            ref={containerRef}
            className={`absolute inset-0 flex items-center justify-center opacity-0 overflow-hidden cursor-pointer ${
              !ready ? "hidden" : ""
            } ${busy ? "pointer-events-none" : ""}`}
          />
        )}
      </div>
    </div>
  );
}
